...
# ── No-arg on empty state prints nothing ──────────────────────────────────────
output=$(omarchy-monitor-base 2>/dev/null)
[[ -z "$output" ]] || fail "no-arg on empty state prints nothing" "got: $output"
pass "no-arg on empty state prints nothing"
...
# ── no-arg now prints the persisted bases ─────────────────────────────────────
output=$(omarchy-monitor-base 2>/dev/null)
[[ "$output" == *"HDMI-A-1 0"* && "$output" == *"DP-1 10"* ]] \
  || fail "no-arg reads persisted bases" "got: $output"
pass "no-arg reads persisted bases"
